...
### 3. 运算符二次校验 (Operator Enforcement)

Bing 有时会忽略 `filetype:`、`inurl:` 等高级语法而返回语义相关的无关结果。LeakDetector 在客户端对每条返回 URL 进行二次校验：

* **filetype/ext 强制过滤**: 确保 `filetype:xlsx` 仅收录 `.xlsx` 文件 URL，不符合扩展名的链接直接丢弃。
* **inurl 强制过滤**: 确保 `inurl:swagger` 返回的 URL 路径中确实包含目标关键词。
* **噪音率预警**: 当 Bing 返回结果中超过 70% 不符合运算符约束时，自动输出精度警告，提示切换浏览器模式或更换代理。

### 4. 深度内容分析与提取 (Deep Analysis)
...
### 6. 断点续扫 (Resume from Checkpoint)

长时间扫描因验证码、代理中断或手动停止后，无需从头重跑：

* **自动检测未完成会话**: 每次点击"开始扫描"时，自动检测上次未完成的扫描任务。
* **弹窗询问**: 若存在未完成任务，弹出提示框显示进度（如"已完成 45/150 条"），由用户决定是否继续。
* **精准跳过**: 仅跳过已执行的 Dork 语法，从断点处无缝继续，原有报告完整保留。
* **检查点文件**: 进度实时写入 `results/Scan_xxx/progress.json`，即使异常崩溃也能恢复。
...
1. 扫描过程中点击 `■ 停止` 或意外中断后，进度已自动保存至 `results/Scan_xxx/progress.json`。
2. 再次点击 `▶ 开始扫描`，工具会自动弹出提示：
   > 发现上次未完成的扫描（进度: 45/150 条），是否从断点继续？
3. 点击**是**：从断点继续，已执行的 Dork 自动跳过，续扫结果保存为 `信息泄露扫描报告_resumed_xxx.xlsx`（原报告保留）。
4. 点击**否**：忽略断点，开始全新扫描。
...
**推荐方式**: 点击主界面的 `关键词管理` 按钮，通过可视化界面进行编辑。
...
* **`信息泄露扫描报告_xxx.xlsx`**: 核心产出物，包含以下 Sheet 页：
  * **汇总详情页**: 包含所有发现的高危条目，按风险评分排序。红色背景为极高危（身份证/密码），黄色为中危（邮箱/手机）。
  * **扫描概览页**: 各类风险的数量统计、高危域名 Top20 分布、下载统计。
  * **全量URL页**: 本次扫描命中的**所有 URL 完整地址**，按目标域名分组、敏感命中条目优先排列，便于快速人工复核与二次利用。
* **下载的文件**: 扫描过程中自动下载的 `.xlsx`, `.pdf` 等原始文件，保存在对应子文件夹中，供人工取证。
* **`progress.json`**: 断点续扫检查点文件，记录已完成的 Dork 列表，异常中断后可恢复进度。
...
* 续扫的新结果保存为 `信息泄露扫描报告_resumed_YYYYMMDD_HHMMSS.xlsx`，与原报告存放在同一 session 目录下（`results/Scan_xxx/`）。
* 原始报告完整保留，两份报告均可查阅。
* 若希望合并两次结果，可手动复制"汇总详情"和"全量URL" Sheet 页内容到同一文件。

**Q7: 全量URL Sheet 的用途是什么？**

* 汇总了本次扫描对目标域名发现的**所有完整 URL**，包括未命中敏感信息的普通页面。
* 适合作为二次渗透的资产清单，或传递给其他工具（如 httpx/nuclei）做进一步批量探测。
* 结果按目标域名分组，命中敏感信息的 URL 排在前面，便于优先处理。
